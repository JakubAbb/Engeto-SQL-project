...
WHERE 
	YEAR(ci.date_from) >= 2006
	AND YEAR(ci.date_from) <= 2018
...
-- ceny a platy za roky 2006-2018 pro celou republiku, pro jednotlivé kategorie zboží a pro jednotlivá odvětví

CREATE TABLE IF NOT EXISTS t_jakub_abbrent_project_SQL_primary_final
(
	WITH products AS (
		-- ceny za rok 2006-2018 pro celou republiku, pro jednotlivé kategorie zboží
		SELECT 
		YEAR(ci.date_from) AS `date`,
		cpc.name AS product_name,
		cpc.price_value AS quantity, 
	...
	salaries AS (
		-- platy za rok 2006-2018 pro celou republiku, pro jednotlivá odvětví
		SELECT 
		ca.payroll_year AS `date`, 
		cpib.name AS industry_name,
		ROUND(AVG(ca.value), 2) AS industry_avg_salary, -- průměrné platy
...
-- množství produktu průměrně za celou republiku pro roky 2006 a 2018
WITH salaries AS (
	SELECT DISTINCT 
		tab1.`date`, 
...
products AS (
	...
	FROM t_jakub_abbrent_project_SQL_primary_final tab1
	WHERE
		(tab1.product_name = 'Mléko polotučné pasterované' 
...
WITH product_price_growth AS ( 
	WITH product1 AS (
		SELECT DISTINCT 
			tab1.`date`, 
			ROUND(AVG(tab1.product_avg_price), 2) AS all_products_price
		FROM t_jakub_abbrent_project_SQL_primary_final tab1
		GROUP BY tab1.`date`  
	),
	product2 AS (
