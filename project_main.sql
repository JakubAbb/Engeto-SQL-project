...
-- ceny a platy za roky 2006-2014 pro celou republiku, pro jednotlivé kategorie zboží a pro jednotlivá odvìtví
...
-- -- množství produktu na jednotlivá odvìtví pro roky 2006-2014
-- SELECT 
-- 	tab1.*,
...
-- množství produktu prùmìrnì za celou republiku pro roky 2006 a 2014
WITH salaries AS (
	SELECT DISTINCT 
		tab1.`date`, 
...
-- data pouze pro Èeskou republiku
SELECT 
-- 	tab2.*,
	tab2.`year`, tab2.GDP 
FROM t_jakub_abbrent_project_sql_secondary_final tab2
...
SELECT 
	...
	round((hdp1.GDP - hdp2.GDP) / hdp2.GDP * 100, 2) AS hdp_growth_percent -- výpoèet meziroèního nárùstu HDP
FROM hdp1
JOIN hdp2
